...
Work data is stored in the `work-log/` directory (or location specified in `work-chronicler.yaml`):
...
**Important**: If `work-log/filtered/` exists, use it instead of the main work-log directories. This contains a pre-filtered subset of significant work (e.g., filtered by organization, impact level, or linked tickets). The filtered data is more relevant for summarization.
...
3. **Check for supporting documents** (optional context):
   - `work-log/performance-reviews/` - Past reviews for format/context
   - `work-log/resumes/` - Existing resume for tone
   - `work-log/notes/` - User's own notes and goals
...
1. **Authentication Overhaul** - 5 PRs, 4 tickets (DWP-575, DWP-576, DWP-577, DWP-578)
2. **Eater App Migration** - 4 PRs, 3 tickets (DWP-463, DWP-529, DWP-530)
```
