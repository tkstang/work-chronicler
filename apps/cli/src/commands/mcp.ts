...
export const mcpCommand = new Command('mcp')
  ...
  .action(async (options: { info?: boolean }) => {
    ...
    try {
      const { createServer, startServer } = await import(
        '@work-chronicler/mcp-server'
      );
      ...
      if (
        ...
        console.error(
          'Error: @work-chronicler/mcp-server not found. Install it with:',
        );
        console.error('  npm install @work-chronicler/mcp-server');
      } else {
